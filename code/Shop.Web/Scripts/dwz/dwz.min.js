...
.find("input").attr("checked", !cked);
            ...
            if (cAttr) {
                ...
                if(op.isLast) $(node)._checkParent();
            }
        },
        _checkParent: function () {
            ...
            var $checkbox = $(":checkbox:first", parent);
...
function initLayout() {
    ...
    $("#sidebar .accordion").height(iContentH - 27);
...
function initUI(_box) {
    ...
    if ($.fn.xheditor) {
        $("textarea.editor", $p).each(function () {
            var $this = $(this);
            var op = { plugins: my_xheditor_plugins, html5Upload: false, skin: $this.attr("skin") || "default", tools: $this.attr("tools") || 'full', onUpload: DWZ.jsonEval($this.attr("onUpload")) };
    ...
    if ($.fn.uploadify) {
        $(":file[uploaderOption]", $p).each(function () {
            var $this = $(this);
            //将3.2参数格式 适配到 2.1.4版本 tomCat 2014-4-16
            var options = {
                uploader: '/Scripts/uploadify/scripts/uploadify.v2.1.4.swf',
                buttonImg: '/Scripts/uploadify/img/button.gif',
                fileDataName: $this.attr("name") || "filedata",
                expressInstall: '/Scripts/uploadify/scripts/expressInstall.swf',
                wmode: 'transparent',
                onError: uploadifyError
            };
            var uploaderOption = DWZ.jsonEval($this.attr("uploaderOption"));
            //$.extend(options, uploaderOption);
            options.auto = uploaderOption.auto;
            options.multi = uploaderOption.multi;
            options.script = uploaderOption.uploader;
            if (uploaderOption.fileSizeLimit.toLowerCase().indexOf('kb')) {
                uploaderOption.fileSizeLimit = parseInt(uploaderOption.fileSizeLimit) * 1024;
            }
            options.sizeLimit = uploaderOption.fileSizeLimit;
            options.fileExt = uploaderOption.fileTypeExts;
            options.onComplete = function (event, queueID, fileObj, response, data) {
                uploaderOption.onUploadSuccess(event, response, true);
            };
            options.fileDesc = "请选择文件(" + options.fileExt + ")";
            DWZ.debug("options: " + DWZ.obj2str(options));
    ...
    /*表单验证错误提示方式绑定 使用MVC实体验证 jquery.validate.unobtrusive的 tomCat 2014-1-13
    ...
    /*表单绑定MVC实体验证 tomCat  2014-1-13 */
    if ($.validator.unobtrusive) { $.validator.unobtrusive.parse($p); }
    ...
    //});
    //}
    /* 添加 my97 日期控件 tomCat 2014-4-16*/
    if (WdatePicker) {
        $('input.wdate,input.date', $p).each(function () {
            var $this = $(this);
            $this.focus(function () {
                var opts = {};
                ...
                if ($this.attr("minDate")) opts.minDate = $this.attr("minDate");
                if ($this.attr("maxDate")) opts.maxDate = $this.attr("maxDate");
                opts.onpicked = function () {
                    $this.keyup();
                }
                WdatePicker(opts);
            });
            $this.parent().find("a.inputDateButton").click(function () {
                $this.focus();
            });
        });
        $('a.inputDateButton', $p).each(function () {
            var $this = $(this);
            $this.click(function () {
                $this.prev('input.wdate,input.date').focus();
            });
        });
    }
    ...
    reload: function (url, options) {
        ...
        if ($panel) {
            ...
            if ($tab.hasClass("external")) {
                ...
                $panel.ajaxUrl({
                    type: "GET", url: url, data: op.data, callback: function (response) {//POST 改为 GET tomCat 2014-11-18
        ...
        tabs: function (options) {
            ...
            function initTab(jT) {
                ...
                jTabs.each(function (iTabIndex) {
                    ...
                    $("a", this).click(function(event){
                         //修复当默认选中的tab是ajax的时候，不会ajax的bug tomCat 2013-11-26
            ...
            function switchTab(jT, iTabIndex) {
                ...
                if (op.reverse && (jTab.hasClass("selected"))) {
                    ...
                    jTab.addClass("selected");
                     //修复当默认选中的tab是ajax的时候，不会ajax的bug tomCat 2013-11-26
                    $("a."+op.ajaxClass, jTab).each(function(){
...
function navTabPageBreak(args, rel) {
    if(!rel) $("form",navTab.getCurrentPanel()).find("input[name='pageNum']").val(1);
...
function uploadifyError(event, queueId, fileObj, errorObj) {
    switch (errorObj.type) {
        case "File Size":
            alertMsg.error("文件大小超过" + (errorObj.info / 1024) + "KB");
            break;
        case "File Type":
            alertMsg.error("文件类型不正确");
            break;
        default:
...
$.fn.extend({
    ...
    $.extend({
        bringBackSuggest: function (args) {
            ...
            $box.find("[dataField]").each(function () {
                var $input = $(this);
                ...
                for (var key in args) {
                    ...
                    if (dataField == inputDataField) {
                        $input.val(args[key]).blur().change();
                        break;
                    }
                }
            });
