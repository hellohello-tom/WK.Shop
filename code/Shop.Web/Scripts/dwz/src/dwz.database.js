...
(function($){
	...
	$.extend({
		bringBackSuggest: function(args){
			...
			$box.find(":input[dataField]").each(function(){
				var $input = $(this);
				//var inputName = $input.attr("name");
				var inputDataField=$input.attr("dataField");
				for (var key in args) {
					var dataField = (_lookup.pk == key) ? _util.lookupPk(key) : _util.lookupField(key);
					if (dataField == inputDataField) {
						$input.val(args[key]);
						break;
					}
				}
			...
			return this.each(function(){
				...
				function _show(event){
					...
					postData[$input.attr("postField")||"inputValue"] = $input.val();
				 	var isNeedAjax=$suggest.find("li").filter(function(){
                       //return $input.val()!="" && $(this).text().indexOf($input.val())>-1;
                       return false;
                    }).size()==0;//从下拉框里找不到当前文本框的值时，才去服务器取数据 tomCat	
                    if(isNeedAjax) {		
						$.ajax({
							global:false,
