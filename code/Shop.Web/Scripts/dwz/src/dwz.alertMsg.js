...
var alertMsg = {
	...
	_open: function(type, msg, buttons){
		...
		if (this._types.info == type || this._types.correct == type){
			...
			//当鼠标放上去的时候，不关闭，移开鼠标才关闭 tomCat 
            $(this._boxId).mouseover(function () {
                clearTimeout(alertMsg._closeTimer);
                alertMsg._closeTimer = null;
            }).mouseout(function () {
                alertMsg._closeTimer = setTimeout(function () { alertMsg.close() }, 1000);
            });
		} else {
			$(this._bgId).show();
		}
